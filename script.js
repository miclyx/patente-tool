    ...
    function addPhraseTranslationToText(text) {
        ...
        phrasesTranslation.forEach(phrase => {
            const regex = new RegExp(`(${phrase['原文']})`, 'g');
            updatedText = updatedText.replace(regex, `<span class="translatable" style="text-decoration: underline; cursor: pointer;" onmouseover="showTooltip(event, '${phrase['翻译']}')" onmouseout="hideTooltip()">$1</span>`);
        });
        return updatedText;
    }
    ...
    window.onload = function() {
        if (document.getElementById('category-list')) {
            ...
            if (document.title.includes('短语翻译')) {
